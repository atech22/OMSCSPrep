...
def write_movie_info(filename, movietitles):
    movies_output = ""
    for title, actors in sorted(movietitles.items()):
        actors = sorted(actors)
        actors = ", ".join(actors)
        movies_output += title + ": " + actors + "\n"
    print(movietitles)
    
    
    try:
        file_writer = open(filename, "w")
        file_contents = file_writer.write(movies_output)
    except:
        return "An error writing to the file has occurred"
    finally:
        file_writer.close()
