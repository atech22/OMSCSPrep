...
def fancy_find(search_within, search_for):
    #print(search_within.find(search_for))
    if search_within.find(search_for) != -1:
        return str(search_for) + " found at index " + str(search_within.find(search_for))+"!"
    else:
        return str(search_for) + " was not found within " + str(search_within) + "!"
